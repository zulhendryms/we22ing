...
class ReportCustomerController extends Controller
{
    ...
    public function report(Request $request)
    {
        ...
        switch ($reportname) {
            case 'customer1':
                $reporttitle = "Report Customer Group By City ";
                $pdf = SnappyPdf::loadView('AdminApi\Report::pdf.'.$reportname, compact('data', 'user', 'reportname','filter', 'reporttitle'));
            break;
            case 'customer2':
                $reporttitle = "Report Customer Group By City (Incl. Address)";
                $pdf = SnappyPdf::loadView('AdminApi\Report::pdf.'.$reportname, compact('data', 'user', 'reportname','filter', 'reporttitle'));
            break;
            case 'customer3':
                $reporttitle = "Report Customer Group By Business Partner";
                $pdf = SnappyPdf::loadView('AdminApi\Report::pdf.'.$reportname, compact('data', 'user', 'reportname','filter', 'reporttitle'));
            break;
            case 'customer4':
                $reporttitle = "Report Customer Group By Business Partner (Incl. Address)";
                $pdf = SnappyPdf::loadView('AdminApi\Report::pdf.'.$reportname, compact('data', 'user', 'reportname','filter', 'reporttitle'));
            break;
            case 'customer5':
                $reporttitle = "Report Customer Group By Account";
                $pdf = SnappyPdf::loadView('AdminApi\Report::pdf.'.$reportname, compact('data', 'user', 'reportname','filter', 'reporttitle'));
            break;
        }
    ...
    private function query($reportname) {
        switch ($reportname) {
            default:
                return "SELECT
                        bp.Name, bp.Email,bp.PhoneNumber,bp.Website, co.Code AS Comp,
                        bpg.Name AS BusinessPartnerGroup,
                        CONCAT (c.Name,' - ',c.Code) AS City,
                        CONCAT (bpag.Name,' - ',bpag.Code) AS BusinessPartnerAccountGroup,
                        bpa.Address AS Address,
                        pt.Name AS SalesTerm,
                        t.Name AS SalesTax,
                        cur.Code AS SalesCurrency
                        ...
                        LEFT OUTER JOIN mstcurrency cur ON bp.SalesCurrency = cur.Oid
                        LEFT OUTER JOIN company co ON bp.Company = co.Oid
                        LEFT OUTER JOIN msttax t ON bp.SalesTax = t.Oid
                        LEFT OUTER JOIN mstpaymentterm pt ON bp.SalesTerm = pt.Oid
                        WHERE bp.GCRecord IS NULL AND 1=1 AND bp.IsSales = 1
                        ORDER BY bp.Code
                        LIMIT 50";
        }
        return "";
