...
<body>
    ...
    <table width="100%">
        <thead>
            <tr>
                <th width="80px" class="firstcol">TRANSACTION CODE</th>
                <th width="80px">TRANSACTION NOTE</th>
                <th width="120px">EXP DATE</th>
                <th width="120px">UNITS</th>
                <th width="120px">UNIT PRICE</th>
                <th width="100px" class="lastcol">TOTAL</th>
            </tr>
        </thead>
        <tbody>
        @foreach($data as $row)
        <tr>
            <td>{{$row->Code}}</td>
            <td>{!! $row->Note !!}</td>
            <td>{{date('d/m/y', strtotime($row->DateExpiry))}}</td>
            <td align="right">{{$row->Quantity}}</td>
            <td align="right">{{number_format($row->TotalAmount,2)}}</td>
            @php $total=0; $total += $row->Quantity * $row->TotalAmount; @endphp
