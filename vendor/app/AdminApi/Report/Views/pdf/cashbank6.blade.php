    ...
    <table>
      <thead>
        <tr>
          ...
          <th>CODE</th>
          <th class="date">Account Cash Bank</th>
      ...
      <tbody> 
        ...
        @foreach($data as $row)
          @if ($group != $row->Date)
            ...
            @endif
            <tr><td colspan="7" class="group"><strong>{{date('d M Y', strtotime($row->Date))}}</strong></td></tr>
            @php $group = $row->Date;  $sumdebet=0; $sumcredit=0; $sumBalance=0; @endphp
          @endif
          <tr>
