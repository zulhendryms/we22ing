    ...
    <table>
      <thead>
        <tr> {{--width:675px--}}
          ...
          <th style="width:50px">CUR</th>
          <th style="width:50px">P.TAX</th>
          <th class="lastcol" style="width:50px">P.TERM</th>
        </tr>
      </thead>
      <tbody>
        @php $group=""; @endphp
        @foreach($data as $row)
          @if ($group != $row->BusinessPartnerAccountGroup)
            <tr>
              <td colspan="6" class="group"><strong>{{$row->BusinessPartnerAccountGroup}} ( {{$row->Address}} )</strong></td>
            </tr>
