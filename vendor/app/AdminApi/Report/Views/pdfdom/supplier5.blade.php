...
<body style="margin:0; font-family:Tahoma, Geneva, sans-serif">
  ...
  <main>
    <table>
      <thead>
        <tr> {{--width:675px--}}
          <th class="firstcol" style="width:250px">NAME</th>
          <th style="width:100px">GROUP</th>
          <th style="width:80px">CITY</th>
          <th style="width:80px">CUR</th>
          <th style="width:50px">P.TAX</th>
          <th class="lastcol">P.TERM</th>
        </tr>
      </thead>
      <tbody>
          @php $group=""; @endphp
          @foreach($data as $row)
            @if ($group != $row->BusinessPartnerAccountGroup)
            <tr>
              <td colspan="5" class="group">{{$row->BusinessPartnerAccountGroup}} ( {{$row->Address}} )</td>
            </tr>
            @php $group = $row->BusinessPartnerAccountGroup;  @endphp
            @endif
            <tr>
              <td class="firstcol">{{$row->Name}}</td>
              <td>{{$row->BusinessPartnerGroup}}</td>
              <td>{{$row->City}}</td>
              <td>{{$row->PurchaseCurrency}}</td>
              <td>{{$row->PurchaseTax}}</td>
              <td class="lastcol">{{$row->PurchaseTerm}}</td>
            </tr>
          @endforeach
          {{--
        <tr>
