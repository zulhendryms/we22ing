...
<body style="margin:0; font-family:Tahoma, Geneva, sans-serif">
  ...
  <main>
    <table>
      <thead>
        <tr> {{--width:675px--}}
          <th class="firstcol" style="width:200px">NAME</th>
          <th style="width:80px">CITY</th>
          <th style="width:80px">PHONE</th>
          <th style="width:150px">E-MAIL</th>
          <th class="lastcol">WEB SITE</th>
        </tr>
      </thead>
      <tbody>
          @php $group=""; @endphp
          @foreach($data as $row)
            @if ($group != $row->BusinessPartnerGroup)
            <tr>
              <td colspan="5" class="group">{{$row->BusinessPartnerGroup}} ( {{$row->Address}} )</td>
            </tr>
            @php $group = $row->BusinessPartnerGroup;  @endphp
            @endif
            <tr>
