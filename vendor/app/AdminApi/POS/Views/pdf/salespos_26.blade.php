      ...
      <table>
        <thead>
          <tr> {{--width:675px--}}
            ...
            <th class="lastcol"style="width:100px">SALES PRICE</th>
          </tr>
        </thead>   
        <tbody>
          @php $totalSalesPrice=0; @endphp
          @foreach($data as $row)
            <tr>
              ...
              <td>{{$row->UpdatedAt}}</td>
              <td >{{$row->UpdatedBy}} </td>
              <td class="lastcol" align="right">{{number_format($row->SalesPrice ,2,',','.')}}</td>
              @php $totalSalesPrice = $totalSalesPrice + $row->SalesPrice; @endphp
            </tr>
          @endforeach
          
            <tr>
              <td colspan="7" align="right"><strong>TOTAL</strong></td>
              <td class="total" align="right"><strong>{{number_format($totalSalesPrice ,2,',','.')}}</strong></td>
            </tr>
        </tbody>
      </table>
    </div>
