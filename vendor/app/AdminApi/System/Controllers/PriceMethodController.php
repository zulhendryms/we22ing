...
class PriceMethodController extends Controller
{
    public function index(Request $request)
    {        
        try {
            $type = $request->input('type') ?: 'combo';
            $data = PriceMethod::whereNull('GCRecord');
    ...
    public function show(PriceMethod $data)
    {
        try {            
            return $data;
        } catch (\Exception $e) {
    ...
    {        
        ...
        try {            
            if (!$Oid) $data = new PriceMethod();
            else $data = PriceMethod::findOrFail($Oid);
    ...
    public function destroy(PriceMethod $data)
    {
        try {            
